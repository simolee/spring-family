...
public class CustomerRunner implements ApplicationRunner {
    ...
    private Long orderCoffee() {
        OrderVO orderVO = OrderVO.builder().customer("Li Lei")
                .items(Arrays.asList("capuccino"))
                .build();
        RequestEntity<OrderVO> request = RequestEntity.post(UriComponentsBuilder.
                fromUriString("http://waiter-service/order/").build().toUri())
                .body(orderVO);
